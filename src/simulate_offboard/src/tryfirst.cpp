...
class TARGET_POINT{
    ...
    double y;
    double z;
    double waite_time;//到达之后的等待时间
};
//TARGET_POINT target_point;



  const int num_point = 3;//点数
 double point[num_point][3]={{1.0,2.0,3.0},{0.0,2.0,3.0},{0.0,0.0,3.0}};
//double point[num_point][3]={{0.0,0.0,5}};//点的规定
...
drone_pose[0] = pose_msg->pose.position.x;
drone_pose[1] = pose_msg->pose.position.y;
drone_pose[2] = pose_msg->pose.position.z;

  if(!(ct++%10)){

                ROS_INFO("%lf    %lf    %lf",fabs(drone_pose[0]-point[point_count][0]),fabs(drone_pose[1]-point[point_count][1]),fabs(drone_pose[2]-point[point_count][2]));
            }

    aim_pose_msg.pose.position.x = point[point_count][0];
    aim_pose_msg.pose.position.y = point[point_count][1];
    aim_pose_msg.pose.position.z = point[point_count][2];
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber pose_sub = nh.subscribe<geometry_msgs::PoseStamped>("/mavros/local_position/pose", 200, get_pose_cb);
    ...
    aim_pose_msg.pose.position.x = point[0][0];
    aim_pose_msg.pose.position.y = point[0][1];
    aim_pose_msg.pose.position.z = point[0][2];

    //在进入Offboard模式之前，必须已经启动了local_pos_pub数据流，否则模式切换将被拒绝。
    //这里的100可以被设置为任意数
    for(int i = 100; ros::ok() && i > 0; --i){
        local_pos_pub.publish(aim_pose_msg);
        ros::spinOnce();
        rate.sleep();
    }
    ...
    while(ros::ok())
    {
        ...
        if(land_sign){//如果有降落信号，则切换为land模式

            set_mode_client.call(land_set_mode);

        }else{//如果没有降落信号，则为offboard模式
...
//这里判断是否到达目标点
            if(fabs(drone_pose[0]-point[point_count][0])<distance&&fabs(drone_pose[1]-point[point_count][1])<distance&&fabs(drone_pose[2]-point[point_count][2])<distance+0.1){
            if(point_count+1<num_point)
            {
            //point_count++;
            ROS_WARN("arrive");
            }else{
            ...
            // ROS_WARN("arrived%lf",point[point_count][0]);
    }

        //发布位置信息，所以综上飞机只有先打开offboard模式然后解锁才能飞起来
        local_pos_pub.publish(aim_pose_msg); 
        //  ROS_INFO("target:%lf||%lf||%lf----%d",point[2-point_count][0],point[2-point_count][1],point[2-point_count][2],point_count);
            // ROS_INFO("%lf",drone_pose[1]);
